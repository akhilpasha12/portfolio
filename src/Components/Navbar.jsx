    ...
    <div className="flex z-10 justify-between items-center w-full lg:h-16 md:h-16 h-14 lg:px-16 md:px-16 px-4  bg-[#000] fixed">
      ...
      <ul className="hidden md:flex lg:flex">
        {links.map(({ link, id }) => (
          <li
            key={id}
            className={` ${active === link ? "text-red-600" : "text-white"}  cursor-pointer capitalize font-medium lg:text-base md:text-base text-lg overAlltext px-4 hover:scale-105 duration-200`}
          >
            <Link
              to={link}
              smooth={true}
      ...
      {icon && (
        <ul className="flex flex-col justify-center items-center w-full h-screen bg-gradient-to-b from-black to-gray-400 absolute top-0 left-0  ">
          {links.map(({ link, id }) => (
            <li
              key={id}
              className="text-white cursor-pointer capitalize font-medium text-xl overAlltext px-4 py-4 "
            >
              <Link
                to={link}
                smooth={true}
